...
from src.script.entities.entity import NameableEntity, CreatableFromModuleEntity, CreatableEntity
from src.script.registries.registry import NameableEntityRegistry, CreatableFromModuleEntityRegistry
from src.script.services.base import Service


class ListableEntityService(Service, ABC):
...
class CreatableEntityService(ListableEntityService, ABC):
    def __init__(self, registry, **kwargs):
        super().__init__(registry, **kwargs)
...
class NameableEntityService(CreatableEntityService, ABC):
    ...
    def rename(self, entity: NameableEntity, new_name: str, new_emoji:str) -> Dict[str, str]:
        ...
        if new_emoji is not None and new_emoji and not self._is_emoji(new_emoji):
    ...
    @staticmethod
    def _is_valid_name(name: str, max_length: int = 255) -> tuple[bool, str]:
        """
    ...
    @staticmethod
    def _is_emoji(emoji: str) -> bool:
        """Validate emoji input"""
        if not emoji:
            return True  # Empty is okay

        try:
            # Try to import emoji library for validation
            import is_emoji, emoji_count
            return bool(emoji_count(emoji)) and is_emoji(emoji)
        except ImportError:
            # If emoji library not available, do basic validation
            return len(emoji) <= 2 and emoji.isprintable()

class CreatableFromModuleEntityService(NameableEntityService, ABC):
    ...
    def _init_entity(self, submodule: str, **kwargs) -> Type[CreatableFromModuleEntity]:
        return self.registry.module_loader.load(f'{self.package}.{submodule}', **kwargs)